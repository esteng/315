...
(defun initialize-betrayals (size)
    "(size)
    initializes the betrayals array to be a size by size grid "
    (setf *betrayals* (make-array (list size size) :initial-element 0)))
...
(defun print-betrayals ()
  (loop for x from 0 to 3 do
    (let ((str " ")))
       (loop for y from 0 to 3 do
        (concatenate 'string str (princ-to-string (aref *betrayals* x y)) )
        )
    (format t "~a~%" str)))
...
Run ncycles & analyze last lastn cycles for nruns & other optional parameters."
  (setq *path* "/Users/Elias/315/A3/results/")
  (seed-random)
