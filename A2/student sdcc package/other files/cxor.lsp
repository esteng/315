;;;file: continuous-xor-cessation-learnability 315.cl
;;;purpose: add ability for SDCC to cease learning if significant progress is not made; learnability paradigm only; PSYC 315
;;;programmers: Eric Doty & Thomas R. Shultz
;;;modifications: in random-xor, changed name & use if structure & learnability variable
;;;  1 setf & no optional path in run-learnability-xor-sdcc
;;;  Record structure for each network & for each experiment
;;;  Don't bother printing weights for the last network
;;;  Construct train & test patterns with random-xor & individually for each network
;;;  Make named folders to hold data, 1 for each combination of parameter values
;;;  Run all conditions, 1 for each combination of parameter values
;;;  Print network number & parameter values: learnability, patience, threshold
;;;  Treat these variables as local here, not global: path, learnability, patience, threshold
;;;  But *learning-patience* & *learning-threshold* are needed for cessation in train function so keep them
;;;  Declare undeclared unvariables: outcome, output-epochs
;;;started: 15 nov 11
;;;current: 8 nov 12
